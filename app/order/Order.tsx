...
const Order: React.FC = () => {
  ...
  const [discountBulk, setDiscountBulk] = useState<string>("0"); // <-- NEW
  ...
  const discountBulkInputRef = useRef<TextInput | null>(null); // <-- NEW
  ...
  const totalQty = Object.values(orderQuantities).reduce(
    (acc, val) => acc + val,
    0,
  );
  ...
  // Compute consumer and bulk totals from pack types for display
  // (exact split happens server-side; here we just show totals)
  const discountValue = parseFloat(discount) || 0;
  const discountBulkValue = parseFloat(discountBulk) || 0; // <-- NEW
  ...
  const handleSave = async (): Promise<void> => {
    try {
      ...
      const orderPayload = {
        ...
        discountAmountBulk: parseFloat(discountBulk) || 0, // <-- NEW
  ...
  const renderItem = ({
    item,
    index,
  }: ListRenderItemInfo<Item>): JSX.Element => {
            ...
            <View className="flex-row justify-between items-center">
              <View>
                ...
                <Text className="text-indigo-200 text-sm mt-1">
                  {partyName}{" "}
                  <Text className="text-xs opacity-80">ID: {partyId}</Text>
                </Text>
              </View>
